...
void hcal_detEff( Int_t kine=-1, Int_t mag=-1  ){
  ...
  Double_t confac = 3.; //Number of sigma in delta x/y considered a "detection"
  ...
  TH1D *hW2_badYcut_3s = new TH1D( "hW2_badYcut_3s",";W^2 (GeV);", binfac*W2fitmax, 0.0, W2fitmax );
  TH1D *hW2_badYcut_6s = new TH1D( "hW2_badYcut_6s",";W^2 (GeV);", binfac*W2fitmax, 0.0, W2fitmax );
  TH1D *hW2_badYcut_9s = new TH1D( "hW2_badYcut_9s",";W^2 (GeV);", binfac*W2fitmax, 0.0, W2fitmax );
  TH1D *hW2_badYcut_12s = new TH1D( "hW2_badYcut_12s",";W^2 (GeV);", binfac*W2fitmax, 0.0, W2fitmax );
  ...
  while( progress<1.0 ){
	...
	continue;
      ...
      //Fill dx with dy cut
      if( abs(dy-dy0)<confac*dy_sig ){
	hdx_HCAL_BBcut_dycut->Fill( dx ); //Harm elastics (efficiency numerator)
	if( abs(dx-dx0_p)<confac*dx_sig_p ){
	  hW2_BBcut_HCalcut->Fill( W2 ); //Earm elastics (efficiency denominator)
	}
      }else if( abs(dy-dy0)>confac*dy_sig ){
	hW2_badYcut_3s->Fill( W2 );
	if( abs(dy-dy0)>2*confac*dy_sig ) hW2_badYcut_6s->Fill( W2 );
	if( abs(dy-dy0)>3*confac*dy_sig ) hW2_badYcut_9s->Fill( W2 );
	if( abs(dy-dy0)>4*confac*dy_sig ) hW2_badYcut_12s->Fill( W2 );
      }
...
;
  ...
  Double_t hcalinelastics = (500/harmrange)*hcalbg->Integral(posHCalXi_MC-2*HCalblk_l_v_MC,posHCalXf_MC+2*HCalblk_l_v_MC);
  Double_t hcalelastics = (500/harmrange)*p->Integral(posHCalXi_MC-2*HCalblk_l_v_MC,posHCalXf_MC+2*HCalblk_l_v_MC);
