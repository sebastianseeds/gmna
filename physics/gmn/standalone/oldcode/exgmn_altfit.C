...
//Fit params
const Double_t fitr_l = -1.5;
const Double_t fitr_h = 0.7;
...
double fpeaks(double *x, double*par){
  ...
  for(int p=0; p<npeaks; p++){
    double norm = par[3*p+npol];
    double mean = par[3*p+npol+1];
    double sig = par[3*p+npol+2];
...
void exgmn_altfit( const char *configfilename="setup_extract_gmn_SBS4.cfg", const char *outputfilename="outfiles/gmn_out.root" ){
  ...
  // Declare outfile
  outputfilename = Form("outfiles/gmn_out_SBS%d_mag%d_altfit.root",kine,magSet);
  ...
  //Clone the key histo for tests
  TH1D *hdx_clone = (TH1D*)hdx_HCAL_fcut->Clone("hdx_clone");
  TH1D *hdx_clone2 = (TH1D*)hdx_HCAL_fcut->Clone("hdx_clone2");
  ...
  if( magSet==30 ){
    fitBG = new TF1( "fitBG", BG_fit_30, fitr_l, fitr_h, 3 );
    //}else if( magSet==50){
  }else{
    fitBG = new TF1( "fitBG", BG_fit_50, fitr_l, fitr_h, 3 );
    //}else{
  ...
  double parAll[9];
  TF1 *fitAll = new TF1("fitAll","gaus(0)+gaus(3)+gaus(6)",fitr_l, fitr_h);
  ...
  fpol = new TF1("fpol","pol3",fitr_l, fitr_h);
  fpol->SetLineColor(16);
  npol = fpol->GetNpar();

  cout << "!!!!!!!!!!!!!!" << endl;
  cout << npol << endl;

  double bg_paralt[npol];
  fpol->SetParameters(&bg_paralt[0]);
  hdx_clone->Fit("fpol","0");
  ...
  //double parAll[9];
  TF1 *fitAllalt = new TF1("fitAllalt","pol3+gaus(3)+gaus(6)",fitr_l, fitr_h);
  fitAllalt->SetLineColor(3); 
  fitAllalt->SetLineWidth(2);
  fitAllalt->SetParameters(par);
  hdx_clone->Fit(fitAllalt,"QR+");
  fitAllalt->GetParameters(parAll);
*/
  /*
  ...
  */
  ...
  //Alternative fit method #2
  //TCanvas *a2 = new TCanvas("a1","a1",10,10,1000,900);
  //a2->Divide(1,2);
  //a2->SetGrid();
  c1->cd(2);
  
  TF1 *totalfit = new TF1("totalfit",Tfit_sbs4,fitr_l, fitr_h,9);
  //totalfit->SetNpx(500);
  //hdx_clone2->Fit("totalfit","V+","ep");
  //a2->cd(2);

  totalfit->SetLineWidth(4);
  totalfit->SetLineColor(kMagenta);
  //totalfit->SetParameter(0,100);
  //totalfit->SetParameter(1,-0.4);
  //totalfit->SetParameter(2,0.6);
  totalfit->SetParameter(3,5000);
  totalfit->SetParameter(4,-0.7);
  totalfit->SetParameter(5,0.5);
  totalfit->SetParameter(6,1700);
  totalfit->SetParameter(7,0.);
  totalfit->SetParameter(8,0.3);

  //hdx_clone2->Fit("totalfit","V+","ep");
  hdx_clone2->Fit("totalfit","QR+");

  TF1 *bg = new TF1("bg",BGfit_sbs4,fitr_l, fitr_h,3);
  bg->SetLineColor(kBlack);
  TF1 *p = new TF1("p",Pfit_sbs4,fitr_l, fitr_h,3);
  p->SetLineColor(kRed);
  //p->SetNpx(500);
  TF1 *n = new TF1("n",Nfit_sbs4,fitr_l, fitr_h,3);
  ...
  //Blind the yields
  double UB_p_tot = p->Integral(-2.5,2.5)/hdx_clone2->GetBinWidth(1);
  double UB_n_tot = n->Integral(-2.5,2.5)/hdx_clone2->GetBinWidth(1);
  ...
  legend->AddEntry(p,Form("Proton Fit, Yield: %f", p_tot),"l");
  legend->AddEntry(n,Form("Neutron Fit, Yield: %f", n_tot),"l");
